...
SUM_DAYS_OF_TARGET_MONTH = calendar.monthrange(
    TARGET_YEAR_MONTH[0], TARGET_YEAR_MONTH[1]
)[1]
...
SUM_BLOCKS_OF_CALENDAR = math.ceil(
    (SUM_DAYS_OF_TARGET_MONTH + NUM_FIRST_WEEKDAY_OF_TARGET_MONTH) / len(SMTWTFS)
) * len(SMTWTFS)
WORK_TIME_RADIO_NAME_LIST = [
    f"{iLabel}_{iDay}"
    for iDay in DAYS_OF_TARGET_MONTH_LIST
    for iLabel in ["start", "end"]
]
DAY_STR_LIST = [
    f"({SMTWTFS[getNumWeekday(TARGET_YEAR_MONTH, iDay)]})"
    for iDay in DAYS_OF_TARGET_MONTH_LIST
]  # 曜日のリスト
